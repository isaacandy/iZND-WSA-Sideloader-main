...
msixfolder = os.getenv('LOCALAPPDATA') + "\\Packages\\46954GamenologyMedia.WSASideloader-APKInstaller_cjpp7y4c11e3w\\LocalCache\\Local\\iZND WSA Sideloader"
...
def start(filearg = ""): # For GitHub installs
    ...
    explorerfile = filearg
    global configpath
    configpath = os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader\\config.ini"
    getConfig()
    main()
...
def getConfig():
    ...
    if installsource == "GitHub":
        if not os.path.exists(os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader"):
            os.makedirs(os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader")
...
def checkForUpdates(window):
    try:
        response = requests.get("https://api.github.com/repos/isaacandy/WSA-Sideloader/releases/latest")
    ...
    try:
        ...
        if source == "GitHub":
            with zipfile.ZipFile(fixPath(fname),"r") as zip_ref:
                if os.path.exists(fixPath(os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest())) == False:
                    zip_ref.extractall(fixPath(os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest()))
                location = os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest()
        elif source == "Microsoft Store":
            with zipfile.ZipFile(fixPath(fname),"r") as zip_ref:
                if os.path.exists(fixPath(os.getenv('LOCALAPPDATA') + "\\Packages\\46954GamenologyMedia.WSASideloader-APKInstaller_cjpp7y4c11e3w\\LocalCache\\Local\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest())) == False:
                    zip_ref.extractall(fixPath(os.getenv('LOCALAPPDATA') + "\\Packages\\46954GamenologyMedia.WSASideloader-APKInstaller_cjpp7y4c11e3w\\LocalCache\\Local\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest()))
                location = os.getenv('LOCALAPPDATA') + "\\Packages\\46954GamenologyMedia.WSASideloader-APKInstaller_cjpp7y4c11e3w\\LocalCache\\Local\\iZND WSA Sideloader\\Bundles\\"+sha256_hash.hexdigest()
        else:
            with zipfile.ZipFile(fixPath(fname),"r") as zip_ref:
    ...
    while True:
        ...
        if event == strings["saveButton"]:
            ...
            if installsource == "Microsoft Store":
                ...
                if os.path.exists(os.getenv('LOCALAPPDATA') + "\\iZND WSA Sideloader\\Bundles"):
                    subprocess.Popen('explorer "'+os.getenv('LOCALAPPDATA') + '\\iZND WSA Sideloader\\Bundles"')
                else:
                    window["_NOBUNDLES_"].Update(visible=True)
            else:
...
def main():
    try:
        ...
        if not os.path.exists(os.getenv('LOCALAPPDATA') + "\\Packages\\MicrosoftCorporationII.WindowsSubsystemForAndroid_8wekyb3d8bbwe"):
            if int(platform.win32_ver()[1].split('.')[2]) < 22000:
                ...
                if event == strings["learnMoreButton"]:
                    window.Close()
                    webbrowser.open("https://github.com/isaacandy/iZND-WSA-Sideloader#getting-started",2)
                    sys.exit(0)
        ...
        window = gui.Window('iZND WSA Sideloader', layout,icon=icon,debugger_enabled=False,finalize=True)
        if installsource != "Microsoft Store":
        ...
        while True:
            ...
            if event is None:
                ...
                sys.exit(0)
            if event == '_UPDATE_':
                webbrowser.open("https://github.com/isaacandy/iZND-WSA-Sideloader/releases/latest",2)
            ...
            if event == strings["helpButton"]:
                ...
                while True:
                    ...
                    if event == strings["wsaSettingsButton"]:
                        ...
                        webbrowser.open("https://github.com/isaacandy/iZND-WSA-Sideloader",2)
        ...
        if outLine.startswith("Success"):
            ...
            while True:
                ...
                if event == strings["okButton"]:
                    break
                elif event == strings["reportBugButton"]: # Open iZND WSA Sideloader issues page
                    webbrowser.open("https://github.com/isaacandy/iZND-WSA-Sideloader/issues",2)
